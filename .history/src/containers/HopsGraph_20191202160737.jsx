...
const styles = {
  root: {
    background: 'radial-gradient(ellipse at top, #801515, transparent), radial-gradient(ellipse at bottom, #aa3939, transparent)',
  },
};

/* eslint-disable no-shadow */
const HopsGraph = ({ classes, hops, loadHops }) => {
  useEffect(() => {
    loadHops();
  }, []);
    ...
    (_, pathIndex) => {
      forEach.convert({ cap: false })(
        (hop, index) => {
          ...
          const target = `${hop.type}_${hop.imdbId}_${index}`;
  ...
  return (
    <Paper className={classes.root}>
      {!hops.loading && hops.data.length > 0 &&
        <BaconGraph levelCount={hops.data[0].length} links={links} nodes={nodes} />
      }
    </Paper>
  );
