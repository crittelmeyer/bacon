    ...
    {nodesReady && (
      <svg width="1000" height="1200">
        {map(link => {
          ...
          console.log('1', document.body.getBoundingClientRect().top, source.top);
          const leftOffset = 275; // dont know why

          return (
            <line
              key={`${source.left}_${source.top}_${target.left}_${target.top}`}
              x1={source.left - leftOffset}
              y1={source.top - (document.body.getBoundingClientRect().top + ((GRAPH_HEIGHT / (levelCount * 1.5)) / 2))}
              x2={target.left - leftOffset}
              y2={target.top - (document.body.getBoundingClientRect().top) + ((GRAPH_HEIGHT / (levelCount * 1.5)) / 2))}
              stroke="black"
            />
          );
        }, links)}
