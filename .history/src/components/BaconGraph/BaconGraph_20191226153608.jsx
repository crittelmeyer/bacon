    ...
    {nodesReady && (
      <svg width="1000" height="1200">
        {map(link => {
          ...
          const topOffset = document.body.getBoundingClientRect().topl;

          return (
            <line
              key={`${source.left}_${source.top}_${target.left}_${target.top}`}
              x1={source.left - leftOffset}
              y1={(source.top - topOffset) + ((GRAPH_HEIGHT / (levelCount * 1.5)) / 2)}
              x2={target.left - leftOffset}
              y2={(target.top - topOffset) + ((GRAPH_HEIGHT / (levelCount * 1.5)) / 2)}
              stroke="black"
            />
          );
        }, links)}
