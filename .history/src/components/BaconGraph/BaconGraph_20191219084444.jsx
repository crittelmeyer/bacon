...
const BaconGraph = ({ classes, levelCount, links, nodeRefs, nodesGrouped, nodesReady, onCloseDrawer, onGotNodeRef, onNodeClick, openDrawer }) => {
  console.log(openDrawer);
  return (
    <div className={classes.root}>
      <Drawer anchor="right" open={!isEmpty(openDrawer)} onClose={onCloseDrawer}>
                ...
                {map.convert({ cap: false })(
                  (node, nodeIndex) => (
                    <BaconNode
                      key={node.id}
                      node={node}
                      selected={openDrawer === node.label}
